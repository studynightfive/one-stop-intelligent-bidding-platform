      ...
      <Card className="!border-[#E2E8F0] !shadow-none mb-4" styles={{ body: { padding: 24 } }}>
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-lg bg-[#F8FAFC] p-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <Button icon={<Save size={14} />} onClick={savePortalDraft} data-testid={PORTAL_TEST_IDS.portalSaveDraft}>保存草稿</Button>
            {finalSubmissions[selectedSupplier] && <Button icon={<Download size={14} />} onClick={downloadReceipt}>下载提交回执</Button>}
            <Button type="primary" disabled={!allSubmitted || finalSubmissions[selectedSupplier]} onClick={confirmFinalSubmission} className="!bg-[#2563EB]" data-testid={PORTAL_TEST_IDS.portalSubmit}>{finalSubmissions[selectedSupplier] ? '已正式提交' : '完成材料提交'}</Button>
          </div>
        </div>
      </Card>
      ...
      <Card className="!border-[#E2E8F0] !shadow-none mb-4" styles={{ body: { padding: 24 } }}>
        ...
        <div className="mt-4 border border-[#BFDBFE] bg-[#EFF6FF] rounded-lg p-4">
          <div className="flex flex-col sm:flex-row sm:items-end gap-3">
            ...
            <Button type="primary" onClick={submitQuote} className="!bg-[#2563EB]" data-testid={PORTAL_TEST_IDS.portalQuoteSubmit}>提交本轮报价</Button>
          </div>
