...
function useIsDemo() {
  try {
    const raw = sessionStorage.getItem('bid-platform-access-token') || localStorage.getItem('bid-platform-access-token')
    return !raw
  } catch { return true }
}
...
export default function SupplierPortal() {
  ...
  useEffect(() => {
    ...
    async function loadFromApi() {
      ...
      const [mats, prs, supps, acts] = await Promise.all([
        fetchPortalMaterials(), fetchPortalPriceRounds(), fetchPortalNotices(), fetchPortalActivity(),
      ])
      if (cancelled) return
      setMaterials(mats)
      setPriceRoundData(prs.map((pr: any) => ({
        round: pr.roundNumber, title: pr.title,
      ...
      setRecords(mats.map((m: any) => ({
        id: `SMR-${m.id}`, supplierId: ctx.supplier.id, supplierName: ctx.supplier.name,
        material: m.name, fileName: m.file?.fileName ?? '',
        fileSize: m.file?.size ? `${(m.file.size / 1024 / 1024).toFixed(1)}MB` : '',
        submitTime: m.submittedAt ? dayjs(m.submittedAt).format('YYYY-MM-DD HH:mm:ss') : '',
        status: m.status === 'submitted' ? 'submitted' : 'missing', round: 1,
      })))
    ...
    loadFromApi().catch(() => { if (!cancelled) loadFromMock() }).finally(() => { if (!cancelled) setLoading(false) })
  ...
  const confirmFinalSubmission = () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        if (!isDemo) {
          try { const idemKey = `submit-${Date.now()}`; await submitPortalMaterials(idemKey); setFinalSubmitted(true); message.success('材料已正式提交，采购方将收到通知'); return } catch (err: any) { message.error(err?.message || '提交失败，请重试'); return }
        }
    ...
    return (
      <div data-testid={PORTAL_TEST_IDS.portalClosed} className="min-h-[600px] flex items-center justify-center p-6">
        <div className="max-w-[480px] text-center">
          ...
          <Alert type="info" showIcon message="所有提交通道已关闭" description={closedEvaluationExample.message} className="!rounded-lg !text-left mb-6" />
          ...
          <Button type="link" onClick={() => setMode('active')} className="!text-[#2563EB]"><ArrowLeft size={14} className="mr-1" /> 返回查看提交记录</Button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div data-testid={PORTAL_TEST_IDS.portal} className="min-h-screen bg-[#F8FAFC] px-4 py-5 sm:p-6">
      ...
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-2"><div className="w-8 h-8 rounded-lg bg-[#2563EB] text-white flex items-center justify-center font-semibold">标</div><div><p className="text-sm font-semibold text-[#1E293B]">智标云 · 供应商提交门户</p><p className="text-xs text-[#64748B]">{invitedSupplierId ? '受邀供应商专属入口 · 提交行为将记录' : isDemo ? 'Demo 模式 · API 不可用时自动降级' : 'API 模式 · 服务端数据'}</p></div></div>
        <Tag color={invitedSupplierId ? 'green' : isDemo ? 'orange' : 'blue'}>{invitedSupplierId ? '专属邀请入口' : isDemo ? 'Demo 降级模式' : 'API 在线模式'}</Tag>
      </div>
      ...
      <Card className="!border-[#E2E8F0] !shadow-none mb-4" styles={{ body: { padding: 24 } }}>
        ...
        <Progress percent={Math.round((submittedCount / totalCount) * 100)} strokeColor="#2563EB" className="!mb-4" format={() => `${submittedCount}/${totalCount}`} />
        ...
        <Table dataSource={requiredMaterialDefs} rowKey="id" pagination={false} size="middle" columns={[
          ...
          { title: '状态', width: 100, render: (_: any, record: any) => { const s = findSubmission(record.name); return s?.status === 'submitted' ? <Tag color="green">已提交</Tag> : <Tag color="red">未提交</Tag> } },
        ...
        <Upload.Dragger multiple showUploadList={false} beforeUpload={batchUpload} className="!mt-4"><UploadCloud size={32} className="text-[#94A3B8] mx-auto mb-2" /><p className="text-sm text-[#64748B]">点击或拖拽文件到此处批量上传</p><p className="text-xs text-[#94A3B8] mt-1">{isDemo ? 'Demo 会自动将文件匹配到所有待提交项目' : '支持多文件批量上传'}</p></Upload.Dragger>
            ...
            return (
              <div key={round.round} className="border border-[#E2E8F0] rounded-lg p-4">
                ...
                {myPrice ? (
                  ...
                  <div className="pl-9">{isCurrent ? <div className="flex items-center gap-3"><Input value={quoteValue} onChange={event => setQuoteValue(event.target.value)} placeholder="请输入报价金额（元）" size="large" prefix="¥" className="!rounded-lg !max-w-[280px]" /><Button onClick={submitQuoteAction} type="primary" size="large" className="!rounded-lg !bg-[#2563EB]">提交报价</Button></div> : <p className="text-sm text-[#94A3B8]">未参与本轮报价</p>}</div>
                )}
              </div>
            )
          })}
      ...
      <div className="text-center pb-6"><Button type="link" onClick={() => setMode('closed')} className="!text-[#64748B]"><Lock size={14} className="mr-1" /> 查看评标结束页面（Demo）</Button></div>
