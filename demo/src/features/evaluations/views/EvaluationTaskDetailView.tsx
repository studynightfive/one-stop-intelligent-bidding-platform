...
export default function EvaluationTaskDetail() {
  ...
  return (
    <div data-testid={EVAL_TEST_IDS.taskDetail} className="p-6">
      {/* Back button + title */}
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between mb-4">
        <div className="flex items-center gap-3">
        <Button type="text" icon={<ArrowLeft size={18} />} onClick={() => navigate('/evaluation')} className="!px-2" data-testid={EVAL_TEST_IDS.taskBack} />
        <div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button loading={aiRunning} onClick={runAiReview} icon={<Bot size={14} />} data-testid={EVAL_TEST_IDS.taskAiReview}>发起 AI 初审</Button>
          <Button onClick={() => { setActiveTab('pricing'); message.info('可在报价页发起新一轮报价') }} icon={<DollarSign size={14} />}>新一轮报价</Button>
          <Button danger disabled={task?.status === 'closed'} onClick={closeEvaluation} icon={<Gavel size={14} />} data-testid={EVAL_TEST_IDS.taskClose}>{task?.status === 'closed' ? '评标已关闭' : '关闭评标'}</Button>
        </div>
      </div>
