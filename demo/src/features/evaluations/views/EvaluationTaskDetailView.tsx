...
export default function EvaluationTaskDetailView() {
  ...
  return <main data-testid={EVAL_TEST_IDS.taskDetail} className="p-4 sm:p-6">
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      ...
      <Space wrap><Button icon={<RefreshCw size={14} />} onClick={() => void load()}>刷新</Button><Button loading={actionLoading === 'ai'} icon={<Bot size={14} />} onClick={() => void runAiReview()} data-testid={EVAL_TEST_IDS.taskAiReview}>发起 AI 初审</Button><Button icon={<DollarSign size={14} />} onClick={() => { setActiveTab('pricing'); setRoundOpen(true) }}>新一轮报价</Button><Button danger disabled={detail.status === 'closed'} icon={<Gavel size={14} />} onClick={closeTask} data-testid={EVAL_TEST_IDS.taskClose}>{detail.status === 'closed' ? '评标已关闭' : '关闭评标'}</Button></Space>
    </div>
    ...
    <Card className="mt-4 !border-[#E2E8F0] !shadow-none">
      ...
      {activeTab === 'submissions' && <SubmissionPanel detail={detail} invites={invites} notices={notices} mockMode={mockMode} onReload={load} onOpenSubmissions={async supplierId => { setSubmissionSupplier(supplierId); setSubmissions(mockMode ? [] : await fetchSupplierSubmissions(evaluationId, supplierId)) }} />}
      {activeTab === 'pricing' && <PricingPanel rounds={rounds} comparison={comparison} mockMode={mockMode} onCreate={() => setRoundOpen(true)} onClose={async roundId => { if (mockMode) setRounds(previous => previous.map(row => row.id === roundId ? { ...row, status: 'closed' } : row)); else { await closePriceRound(evaluationId, roundId); await load() } }} />}
      ...
      {(activeTab === 'technical' || activeTab === 'commercial') && <ScoringPanel category={activeTab} detail={detail} scores={scores} drafts={scoreDrafts} setDrafts={setScoreDrafts} mockMode={mockMode} onSave={async score => { const draft = scoreDrafts[scoreKey(score)]; if (!draft?.value) return; if (mockMode) setScores(previous => previous.map(row => scoreKey(row) === scoreKey(score) ? { ...row, humanScore: draft.value, finalScore: draft.value, adjustmentReason: draft.reason } : row)); else { const updated = await updateScore(evaluationId, score, draft.value, draft.reason || '人工复核调整'); setScores(previous => previous.map(row => scoreKey(row) === scoreKey(score) ? updated : row)) } message.success('人工复审评分已保存') }} onConfirm={async () => { if (!mockMode) await confirmScores(evaluationId, undefined, `${activeTab === 'technical' ? '技术' : '商务'}评分复核完成`); message.success('本类评分已确认') }} />}
    ...
    <Modal title="新一轮报价" open={roundOpen} onCancel={() => setRoundOpen(false)} okText="创建报价轮次" onOk={async () => {
...
function SubmissionPanel({ detail, invites, notices, mockMode, onReload, onOpenSubmissions }: { detail: EvaluationTaskDetail; invites: SupplierInviteSummary[]; notices: SupplementNotice[]; mockMode: boolean; onReload: () => Promise<void>; onOpenSubmissions: (supplierId: string) => Promise<void> }) {
  ...
  return <div className="space-y-4">
    ...
    <Table rowKey="id" dataSource={detail.suppliers} columns={[
      ...
      { title: '操作', render: (_, row) => <Space wrap><Button size="small" onClick={() => void onOpenSubmissions(row.id)}>查看材料</Button><Button size="small" icon={<Copy size={12} />} disabled={!inviteBySupplier[row.id]} onClick={() => void copyText(inviteBySupplier[row.id]?.inviteUrl || '').then(() => message.success('供应商邀请链接已复制'))}>复制邀请</Button><Button size="small" loading={sending === row.id} icon={<Bell size={12} />} onClick={async () => { setSending(row.id); try { if (!mockMode) { await createSupplementNotice(detail.id, { supplierId: row.id, materialIds: detail.materials.filter(item => item.required).map(item => item.id), deadlineMinutes: detail.reviewSettings.supplementDeadlineMinutes, message: '请按评标要求补充缺失或不合格材料' }); await onReload() } message.success('补充通知已发送并记录留痕') } catch (error) { message.error(error instanceof Error ? error.message : '补充通知发送失败') } finally { setSending('') } }}>发补充通知</Button><Button size="small" disabled={!inviteBySupplier[row.id] || inviteBySupplier[row.id].status === 'revoked'} onClick={async () => { if (!mockMode) { await rotateSupplierInvite(detail.id, row.id, '管理员重新生成邀请链接'); await onReload() } message.success('邀请链接已轮换') }}>轮换链接</Button><Button size="small" danger disabled={!inviteBySupplier[row.id] || inviteBySupplier[row.id].status === 'revoked'} onClick={async () => { if (!mockMode) { await revokeSupplierInvite(detail.id, row.id, '管理员撤销邀请'); await onReload() } message.success('邀请链接已撤销') }}>撤销</Button></Space> },
    ]} />
  </div>
}

function PricingPanel({ rounds, comparison, mockMode, onCreate, onClose }: { rounds: PriceRound[]; comparison: PriceComparison | null; mockMode: boolean; onCreate: () => void; onClose: (roundId: string) => Promise<void> }) {
  return <div className="space-y-4"><div className="flex justify-between"><div className="text-sm text-[#64748B]">报价趋势包含 {comparison?.supplierTrends.length || 0} 家供应商；所有金额由服务端留痕。</div><Button type="primary" icon={<DollarSign size={14} />} onClick={onCreate}>新建报价轮次</Button></div><Table rowKey="id" dataSource={rounds} locale={{ emptyText: '尚未创建报价轮次' }} columns={[{ title: '轮次', render: (_, row) => `第 ${row.roundNumber} 轮` }, { title: '标题', dataIndex: 'title' }, { title: '开始', dataIndex: 'opensAt', render: formatDate }, { title: '截止', dataIndex: 'deadline', render: formatDate }, { title: '状态', dataIndex: 'status', render: value => <Tag color={value === 'open' ? 'green' : 'default'}>{value}</Tag> }, { title: '报价数', dataIndex: 'submissionCount' }, { title: '操作', render: (_, row) => <Button size="small" danger disabled={row.status === 'closed'} onClick={() => void onClose(row.id).then(() => message.success('本轮报价已关闭'))}>{row.status === 'closed' ? '已关闭' : '关闭轮次'}</Button> }]} /><div className="text-xs text-[#94A3B8]">{mockMode ? '模拟模式用于页面演示。' : '真实模式下报价由供应商专属门户提交。'}</div></div>
}
