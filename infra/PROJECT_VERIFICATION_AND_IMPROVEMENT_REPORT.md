...
| CI 只构建镜像，没有执行真实浏览器业务流 | 合并后可能遗漏跨层错误 | 基础设施 Job 检查 Runner Chrome，启动完整栈并运行 E2E，始终清理容器 | 本地等价命令已完整通过，CI 作为合并门禁 |
...
- CI 使用锁文件安装依赖，显式检查 Runner 自带 Chrome，并执行契约、后端、前端、基础设施和真实浏览器流程。
