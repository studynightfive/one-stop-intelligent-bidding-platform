...
from app.domains.pricing.formulas import price_score
...
async def test_quote_deadline_and_money() -> None:
    ...
    with pytest.raises(DomainError):
        await container.pricing.submit_quote(
            portal, round_data["id"], amount="10.1", currency="CNY", idempotency_key=None
        )
    ...
    assert quote == again
    # 非资格供应商
    other = PortalPrincipal(supplier_id=suppliers[1]["id"], evaluation_id=eid, tenant_id="t-1", name="乙")
    with pytest.raises(DomainError) as exc:
        await container.pricing.submit_quote(
            other, round_data["id"], amount="1.00", currency="CNY", idempotency_key="q-2"
        )
    assert exc.value.code == "FORBIDDEN"


def test_lowest_quote_price_formula() -> None:
    assert (
        price_score(
            quote=Decimal("120.00"),
            lowest_quote=Decimal("100.00"),
