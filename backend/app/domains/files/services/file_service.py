...
class FileService:
    ...
    async def record_uploaded_part(
        ...
        # 记录分片
        session.uploaded_parts = session.uploaded_parts + [
    ...
    async def _compose_object_from_parts(
        ...
        # 构建 MinIO compose source 对象
        # MinIO 的 compose_object 需要 SourceObject 对象列表
        sources = []
        ...
        try:
            # 按 MinIO SDK 方式构建 ComposeSource
            from minio.datatypes import ComposeSource

            compose_sources = []
            for source_info in sources:
                src = ComposeSource(
                    bucket_name=source_info["bucket_name"],
                    object_name=source_info["object_name"],
                )
                compose_sources.append(src)
