...
| `AuthenticatedUser` + DBSession | `AuthPrincipal`（路由注入） | L0 依赖覆盖调用 `auth_principal_from_context` | 全部内部 API |
...
from app.domains.evaluations.router import (
    auth_principal_from_context,
    get_actor,
    get_container,
...
# EvaluationStore 是应用级 Demo 状态；M4 服务适配器随请求使用当前 DBSession。
async def m6_container(db: DBSession):
    return build_m6_container_with_m4(db, store=app.state.m6_store, bid_snapshot=m5_port)

async def m6_actor(current_user: AuthenticatedUser):
    return auth_principal_from_context(current_user)

app.dependency_overrides[get_container] = m6_container
app.dependency_overrides[get_actor] = m6_actor
api_router.include_router(evaluations_router)
api_router.include_router(portal_router)
```

M6 不接受调试 Header。生产鉴权只消费 M4 `AuthenticatedUser` 的映射结果。
