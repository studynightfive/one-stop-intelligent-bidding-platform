...
class EvaluationService:
    ...
    async def stats(
        self, actor: AuthPrincipal, *, keyword: str | None = None, status: str | None = None, assignee_id: str | None = None
    ) -> dict[str, int]:
    ...
    async def create_from_bid_task(
        ...
        require_create(actor)
        snapshot = await self.bid_snapshot.get_snapshot(tenant_id=actor.tenant_id, bid_task_id=bid_task_id)
        now = _now()
    ...
    async def publish(
        self, actor: AuthPrincipal, evaluation_id: str, *, idempotency_key: str | None
    ) -> dict[str, Any]:
        if idempotency_key:
        ...
        if not validation["valid"]:
            raise validation_error("发布前校验未通过", field_errors=[
    ...
    async def rotate_invite(self, actor: AuthPrincipal, evaluation_id: str, supplier_id: str, reason: str) -> dict[str, Any]:
        entity = self.store.get_evaluation(evaluation_id, tenant_id=actor.tenant_id)
        require_owner(actor, entity)
    ...
    async def revoke_invite(self, actor: AuthPrincipal, evaluation_id: str, supplier_id: str, reason: str) -> dict[str, bool]:
        entity = self.store.get_evaluation(evaluation_id, tenant_id=actor.tenant_id)
        require_owner(actor, entity)
        supplier = self._supplier(entity, supplier_id)
        if supplier.invite_code_hash:
