...
def get_actor(request: Request) -> AuthPrincipal:
    ...
    if actor is None:
        # 允许集成测试通过 header 注入；生产由 M4 中间件写入
        user_id = request.headers.get("X-Debug-User-Id")
        tenant_id = request.headers.get("X-Debug-Tenant-Id")
        role = request.headers.get("X-Debug-Role", "admin")
        name = request.headers.get("X-Debug-User-Name", "调试用户")
        if not user_id or not tenant_id:
            raise domain_http_exception(DomainError(code="UNAUTHENTICATED", message="未认证"), request=request)
        actor = AuthPrincipal(user_id=user_id, tenant_id=tenant_id, name=name, role=role)  # type: ignore[arg-type]
    return actor


async def _run(request: Request, coro):  # type: ignore[no-untyped-def]
    try:
        return await coro
...
async def evaluations_stats(
    ...
    data = await _run(request, container.evaluations.stats(actor, keyword=keyword, status=status, assignee_id=assigneeId))
...
async def publish_evaluation(
    ...
    data = await _run(
        request, container.evaluations.publish(actor, evaluationId, idempotency_key=idempotency_key)
    )
    return success(data, request=request)
...
async def list_notices(
    ...
    data, meta = await _run(
        request, container.pricing.list_notices(actor, evaluationId, page=page, page_size=pageSize)
    )
    return success(data, request=request, meta=meta)
...
async def list_risks(
    ...
    data = await _run(
        request,
        container.scoring.list_risks(
            actor, evaluationId, severity=severity, decision=decision, supplier_id=supplierId
        ),
    )
    return success(data, request=request)
