...
class M4FileServiceAdapter:
    ...
    async def get_file(self, *, tenant_id: str, file_id: str) -> FileRefSnapshot:
        file_obj = await self._load_file(file_id)
        if str(getattr(file_obj, "tenant_id", "")) not in {"", tenant_id} and str(file_obj.tenant_id) != tenant_id:
            raise KeyError(file_id)
        return await self._to_snapshot(file_obj)
    ...
    async def get_download_url(self, *, file_id: str, expires_minutes: int = 60) -> str | None:
        return await self._service.get_download_url(_as_uuid(file_id), expires_minutes=expires_minutes)

    async def _load_file(self, file_id: str) -> Any:
        # M4 FileService 若提供 get_file 则优先；否则按常见 repository 约定回退
        if hasattr(self._service, "get_file"):
            return await self._service.get_file(_as_uuid(file_id))
        if hasattr(self._service, "get"):
            return await self._service.get(_as_uuid(file_id))
        raise RuntimeError("M4 FileService 缺少 get_file/get，请更新适配器")

    async def _to_snapshot(self, file_obj: Any) -> FileRefSnapshot:
        ...
        download = None
        try:
            preview = await self._service.get_preview_url(file_obj.id, expires_minutes=30)
        except Exception:
            preview = None
        try:
            download = await self._service.get_download_url(file_obj.id, expires_minutes=60)
        except Exception:
            download = None
        return FileRefSnapshot(
            id=str(file_obj.id),
            file_name=str(getattr(file_obj, "file_name", getattr(file_obj, "filename", "file"))),
            mime_type=str(getattr(file_obj, "content_type", getattr(file_obj, "mime_type", "application/octet-stream"))),
