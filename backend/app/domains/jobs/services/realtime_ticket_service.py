...
class RealtimeTicketService:
    ...
    async def create_ticket(
        ...
        expires_at = datetime.now(UTC) + timedelta(seconds=WEBSOCKET_TICKET_TTL_SECONDS)
        return ticket_token, expires_at

    async def validate_ticket(self, ticket_token: str) -> dict[str, Any] | None:
        """验证票据并标记为已使用.
        ...
        return 1
        """
        result = await cast(Awaitable[Any], client.eval(lua_script, 1, key))
