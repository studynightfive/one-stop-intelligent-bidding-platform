...
from app.domains.evaluations.state_machine import assert_evaluation_transition, evaluation_progress_for, evaluation_step_for
...
class ScoringService:
    ...
    async def list_scores(
        ...
        if category:
            items = [i for i in items if criteria_by_id.get(i.criterion_id) and criteria_by_id[i.criterion_id].category == category]
    ...
    async def ranking(self, actor: AuthPrincipal, evaluation_id: str) -> dict[str, Any]:
        ...
        for supplier in entity.suppliers:
            ...
            buckets = {"qualification": Decimal("0"), "technical": Decimal("0"), "commercial": Decimal("0"), "service": Decimal("0")}
            total = Decimal("0.00")
            for item in scores:
    ...
    async def start_report(
        self, actor: AuthPrincipal, evaluation_id: str, formats: list[str]
    ) -> dict[str, Any]:
        entity = self.store.get_evaluation(evaluation_id, tenant_id=actor.tenant_id)
        require_reviewer(actor, entity)
        job = await self.jobs.enqueue(
