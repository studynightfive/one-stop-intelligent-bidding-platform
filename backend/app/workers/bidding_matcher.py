...
class BiddingMatcherTask(M7Task):
    """资质要求 ↔ 投标响应 匹配度评分。"""

    name = "app.workers.bidding_matcher.match"
    scene = "material_match"
    ...
    async def _execute(self, spec: JobSpec, payload: dict[str, Any]) -> JobResult:
        ...
        if output.provider_status == ProviderStatus.OFFLINE:
            return self.make_partial_result(
                job_id=spec.job_id,
                output={"matches": structured.get("matches", [])},
