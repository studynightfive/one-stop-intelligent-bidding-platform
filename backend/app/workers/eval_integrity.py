...
class EvalIntegrityTask(M7Task):
    """文件齐全性 / 必填项验证。"""

    name = "app.workers.eval_integrity.check"
    scene = "evaluation_check"
    ...
    async def _execute(self, spec: JobSpec, payload: dict[str, Any]) -> JobResult:
        ...
        if output.provider_status == ProviderStatus.OFFLINE:
            return self.make_partial_result(
                job_id=spec.job_id,
                output={
                    "completeness": structured.get("completeness", 0),
